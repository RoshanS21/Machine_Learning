...
def NORMALIZE2(training, test):
	min = np.amin(training)
	max = np.amax(training)
	for j in range(len(training[0])-1):
		for i in range(len(training)):
			training[i][j] -= min
			training[i][j] /= max - min
		for i in range(len(test)):
			test[i][j] -= min
			test[i][j] /= max - min
...
def GET_NEIGHBORS(training, test_row, num_neighbors):
	...
	for training_row in training:
		distance = L1_Distance(test_row, training_row)
